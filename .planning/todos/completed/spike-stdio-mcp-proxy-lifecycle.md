...
**Done.** Run as four spikes in `.planning/spikes/` — 001 process identity and spawn timing, 002 the
shutdown grace window, 003 timeout budgets, 004 output limits. All four VALIDATED. Every row of the
table above has a recorded observation with its command and raw log excerpt in the relevant spike
README.

The method turned out to be cheaper than this todo assumed: the `claude` CLI is installed **inside the
devcontainer**, so every observation was scripted against real headless sessions rather than needing a
human to click through the IDE. The extension gap is documented per finding instead of closed.

**Findings 8 and 12 were corrected, not merely promoted.** The grace-window row — flagged here as
"the one that can still invalidate the design" — did not invalidate it: `unlinkSync` uses ~0.1ms of a
measured ~490ms window. But the *ladder* was wrong (SIGINT first, stdin never closed) and
"the SIGKILL path gets nothing" was backwards (abrupt client death is the unbounded path). Findings 4,
5 and 6 confirmed; 13, 14, 15 and 16 are new. All written back into
`.planning/notes/vice-mcp-selector-design.md`, and the Phase 01.2 gate is marked cleared in
`ROADMAP.md`.
