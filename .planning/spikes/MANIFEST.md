...
| 002 | shutdown-grace-window | standard | Given a shutdown handler that busy-writes 100ms progress markers, when a session ends gracefully and when it is killed abruptly, then the log yields the real signal order and the number of ms of synchronous work that completes before death — proving or refuting that a synchronous `unlinkSync` release lands | PENDING | mcp, lifecycle, shutdown, leasing |
