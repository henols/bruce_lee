...
**Plans**: TBD — `/gsd-plan-phase 1.2` sets the real count (the progress table still carries the insertion-time estimate of 5)

**Parallelisation**: limited. The protocol shape (criterion 2) precedes both the broker that writes grants and the proxy that reads them; after it is fixed, host-side broker work and container-side proxy work touch disjoint files (`tools/vice-broker.sh` vs `.claude/mcp/vice/vice-proxy.mjs`) and can proceed in parallel. The narrowing edit in criterion 12 and the fresh-session proof in criterion 13 both strictly follow a working broker.
...
*Phase 1.1's count is now real (4 plans, set by `/gsd-plan-phase 1.1`). Phase 1.2 still carries its insertion-time estimate (0/5); `/gsd-plan-phase 1.2` sets the real count.*
