...
**Success Criteria** (what must be TRUE):
 ...
 14. **No orphaned `x64sc`. Ever — and the mechanism does not depend on the dying process running code.** **SIGKILL and SIGSTOP cannot be caught, blocked or handled**: a process receiving signal 9 executes no handler, no `process.on("exit")`, no `finally`, and in a one-process design there is no supervisor left to be told anything. Any scheme requiring the dying broker to act is unavailable by construction, and this is recorded because it is precisely the class of assumption that reads as obviously correct and is structurally incapable of firing. Three layers: **(L1)** every catchable signal — `SIGTERM`, `SIGINT`, `SIGHUP`, `uncaughtException`, `unhandledRejection`, normal exit — the broker kills every tracked child itself, identity-verified against the pid recorded at spawn, reusing Phase 01.3 criterion 6's verified-kill discipline rather than re-deriving it. **(L2, load-bearing, non-negotiable)** a durable pid registry written **at spawn**, before an instance is ever grantable, plus criterion 7's unconditional identity-verified reap on startup. A stray can exist while the broker is down; it can never be *granted*, because there is no broker to grant it, and it can never be mistaken for continuity, because the reap bumps its epoch and `assertSameMachine()` already turns that into `MachineRestartedError`. **(L3, prevention, answered with evidence)** a cgroup per broker run with `cgroup.kill` on startup is the only route that kills strays the instant the parent dies without reintroducing a per-instance process. `PR_SET_PDEATHSIG` is **rejected**: Node core has no `prctl` binding, so it needs a native addon (a compile-on-host dependency on the machine nobody is watching) or a per-instance wrapper — which is the supervisor process criterion 2 just deleted. cgroup access is host-dependent, so **a negative answer is a real result**: if unavailable, L1+L2 ship and the verdict is recorded.
