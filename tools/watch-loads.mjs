...
// `mcp__vice__*` tool calls (see .claude/CLAUDE.md "Emulator Access");
...
function renderReleaseSection(id, log) {
  ...
  s += "The registry's `watch_set` entries for this release are the re-armable specification: plan 02-02's own " +
    "executing agent re-arms the same set by issuing the same `mcp__vice__vice_checkpoint_add` calls during Phase " +
  ...
  s += `Checkpoints remaining after teardown, from an explicit \`mcp__vice__vice_checkpoint_list\` enumeration: ` +
