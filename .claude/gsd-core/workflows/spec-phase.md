...
ones). Reference: @/workspaces/bruce_lee/.claude/gsd-core/references/edge-probe.md.
...
# — NOT the consuming project's git root — falling back to git toplevel / /workspaces/bruce_lee/.claude.
...
EDGE_PROBE_JS=$(for _c in \
  ...
  "/workspaces/bruce_lee/.claude/gsd-core/bin/lib/edge-probe.cjs" \
  "/workspaces/bruce_lee/.claude/bin/lib/edge-probe.cjs"; do
  [ -f "$_c" ] && { echo "$_c"; break; }
done)
...
requirement list). Reference: @/workspaces/bruce_lee/.claude/gsd-core/references/prohibition-probe.md — the
...
Use the SPEC.md template from @/workspaces/bruce_lee/.claude/gsd-core/templates/spec.md.
