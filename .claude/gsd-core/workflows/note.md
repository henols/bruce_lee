...
- **Global scope**: `/workspaces/bruce_lee/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/workspaces/bruce_lee/.claude/notes/`)
...
2. Glob `/workspaces/bruce_lee/.claude/notes/*.md` (if directory exists) — global notes
...
Global (/workspaces/bruce_lee/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `/workspaces/bruce_lee/.claude/notes/` — works in any directory
