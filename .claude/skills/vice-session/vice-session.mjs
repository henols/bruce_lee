...
// persist between Bash invocations -- every `node tools/vice.mjs ...` call
  ...
  if (existing.present && !isExpired(existing)) {
    throw new Error(
      `a session is already active (id ${existing.session_id}, port ${existing.port}, expires ${existing.expires_at}) -- ` +
        `release it first: \`node tools/vice.mjs session release\`. Refusing to silently steal an active session.`
    );
  }
 ...
 *      resolution step may change what a bare `node tools/vice.mjs ping`
 *      does.
 *
  ...
  if (isExpired(s)) {
    throw new Error(
      `session ${s.session_id} (port ${s.port}) expired at ${s.expires_at} -- refusing to fall back to the ` +
        `default instance silently. Recover with: \`node tools/vice.mjs session release\` then ` +
