...
// persist between Bash invocations -- every `node .claude/skills/vice-session/vice.mjs ...` call
  ...
  if (existing.present && !isExpired(existing)) {
    throw new Error(
      `a session is already active (id ${existing.session_id}, port ${existing.port}, expires ${existing.expires_at}) -- ` +
        `release it first: \`node .claude/skills/vice-session/vice.mjs session release\`. Refusing to silently steal an active session.`
    );
  }
 ...
 *      resolution step may change what a bare `node .claude/skills/vice-session/vice.mjs ping`
 *      does.
 *
  ...
  if (isExpired(s)) {
    throw new Error(
      `session ${s.session_id} (port ${s.port}) expired at ${s.expires_at} -- refusing to fall back to the ` +
        `default instance silently. Recover with: \`node .claude/skills/vice-session/vice.mjs session release\` then ` +
        `\`node .claude/skills/vice-session/vice.mjs session acquire\`.`
    );
  }
    ...
    if (baseline.epoch !== current.epoch) {
      throw new Error(
        `session ${s.session_id} (port ${s.port}): the emulator restarted since this session was acquired -- ` +
          ...
          `trust them. Recover with: \`node .claude/skills/vice-session/vice.mjs session release\` then ` +
          `\`node .claude/skills/vice-session/vice.mjs session acquire\`.`
      );
    }
