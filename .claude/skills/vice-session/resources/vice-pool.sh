...
# code (.claude/skills/vice-session/scripts/vice-pool.mjs) via a registry.json file written on the same
...
(.claude/skills/vice-session/scripts/vice-pool.mjs) over a registry.json file on the shared bind mount
...
cmd_status() {
  ...
  # container-side.
  echo "vice-pool: for actual VICE liveness (not just supervisor-pid liveness), run inside the container: node .claude/skills/vice-session/scripts/vice.mjs pool status"
