...
// ============================================================================
// install-resources.mjs (Task 2, quick-260730-q4b): the deploy-on-first-use
// installer that copies this skill's resources/ into <root>/tools/. Every
// test here drives installResources()/ensureResourcesInstalled() against a
...
test("installResources(): executable bit preserved -- both launchers arrive executable, lib/container-guard.sh non-executable like its tracked source", () => {
  const root = mkdtempSync(join(tmpdir(), "vice-install-modes-"));
  installResources({ root, log: () => {} });
  const supervisor = join(installTargetDir(root), "vice-supervisor.sh");
  const pool = join(installTargetDir(root), "vice-pool.sh");
  ...
  assert.ok(statSync(pool).mode & 0o111, "vice-pool.sh must be deployed executable");
