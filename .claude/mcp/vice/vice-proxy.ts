...
// THIS TASK'S SCOPE ONLY (the tracer): vice_ping plus the three synthetics.
// Plan 03 registers the remaining ~43 manifest tools by literally the same
// buildViceTool() call, with no new architecture needed -- their temporary
// absence from tools/list here is a deliberate, disclosed functionality
// gap (see this plan's own objective), not an architectural one.
const TRACER_MANIFEST_TOOL_NAMES: ReadonlySet<string> = new Set(["vice_ping"]);
...
for (const def of readManifestTools()) {
  if (!TRACER_MANIFEST_TOOL_NAMES.has(def.name) || DENY_LIST.includes(def.name)) continue;
